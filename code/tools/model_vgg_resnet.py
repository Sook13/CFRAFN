...
class VggFeatureBlock(nn.Module):
    ...
    def forward(self, x):
        identity = x
        out = self.conv(x)
        identity = self.shortcut(identity)  # 通过捷径连接调整形状
        out += identity  # 确保形状匹配后再相加
        out = self.relu(out) 
...
class CFRAFN(nn.Module):
    def __init__(self, input_dim_eGeMAPS, input_dim_VGGish, feature_weights, expansion=4, dropout=0.1, conv_out_channels=64, resblock_kernel_size=3,
                 transformed_feature_dim=128, cls_dim=128):
